...
const renderComponent = () => {
  ...
  const payerInput = screen.getByDisplayValue(/누가 결제/);
...
describe("비용 정산 메인 페이지", () => {
  describe("비용 추가 컴포넌트", () => {
    ...
    test("필수 추가에 필수적인 값을 입력하지 않고 '추가' 버튼 클릭 시, 에러 메시지를 노출한다.", async () => {
      const {
        addButton,
        descErrorMessage,
        payerErrorMessage,
    ...
    test("비용 추가에 필수적인 값들을 입력한 후 '추가' 버튼 클릭시, 저장에 성공", async () => {
      ...
      await userEvent.selectOptions(payerInput, "영수"); // 테스트를 돌리기 전에 payerList (멤버들 이름)을 셋팅해야 한다.
      await userEvent.click(addButton);
  ...
  describe("정산 결과 컴포넌트", () => {
    test("정산 결과 컴포넌트가 렌더링 되는가?", () => {
      renderComponent();
      const component = screen.getByText(/정산은 이렇게/);
  ...
  describe("새로운 비용이 입력 되었을 때", () => {
    const addNewExpense = async () => {
      const { dateInput, descInput, payerInput, amountInput, addButton } =
        renderComponent();
      await userEvent.type(dateInput, "2024-07-26");
      await userEvent.type(descInput, "제주도");
      await userEvent.type(amountInput, "30000");
    ...
    test("날짜, 내용, 결제자, 금액 데이터가 정산 리스트에 추가 된다", () => {
      // 새로운 비용을 입력
      const expenseListComponent = screen.getByTestId("expenseList");
      //   const dateValue = within(expenseListComponent).getByText("2024-07-26");
      //   expect(dateValue).toBeInTheDocument();
      const descValue = within(expenseListComponent).getByText("제주도");
    ...
    test("정산 결과 또한 업데이트가 된다.", () => {
      ...
      const transactionTest = screen.getByText(/영희가 영수에게 15000원/);
      expect(transactionTest).toBeInTheDocument();
    });
